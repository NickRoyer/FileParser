...
{
    public class MovieListLinqRepo : IMovieRepo
    {
        ...
        {
            return MovieList.Count(ml => (ml.Year >= startYear && ml.Year <= endYear && ml.Genre == genre));
        }

        public long FindMoviesInGrossReceiptRange(long minGross, long maxGross)
        {
            return MovieList.Count(ml => ml.Gross >= minGross && ml.Gross <= maxGross);
        }
